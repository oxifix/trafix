...
impl Message {
    ...
    pub fn builder(begin_string: BeginString, msg_type: MsgType) -> MessageBuilder<false> {
        let header = Header {
            begin_string,
            body_length: Vec::from(msg_type).len() + 1, // +1 SOH char
            checksum: None,
            msg_type,
            fields: None,
        };
...
impl<const IS_INIT: bool> MessageBuilder<IS_INIT> {
    ...
    pub fn header(mut self, field: Field) -> Self {
        if let Some(fields) = &mut self.inner.header.fields {
            fields.push(field);
            self.inner.header.body_length =
                self.inner.header.body_length + Vec::from(field).len() + 1;
        } else {
